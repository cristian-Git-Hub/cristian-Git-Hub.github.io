...
Modified: 
 - 10/21/24: Added Author and picture information. 
   ...
   function myCycle() {
     ...
     pictures.forEach(function(pic, index) {
      console.log(index)
       ...
       initMax += 33;
       // get picture informaion. 
       getAuthorData(rand,index)
     });
   }

   async function getAuthorData (id,curIndex) {
    console.log('id: '+id)
    console.log('index: ' + curIndex)
    const url = 'https://picsum.photos/id/'+id+'/info'
    const curelement = document.getElementById('pic-'+curIndex)
    //curelement.innerText = url
    const response = await fetch(url)
    const data = await response.json()
    console.log(data)
    console.log( data.author )
    curelement.innerHTML = ' <br><b>Author:</b> ' + data.author + 
    '<br><br> <b>Picture Info: </b><a href="' + data.url + '">Link</a>'  
   }
